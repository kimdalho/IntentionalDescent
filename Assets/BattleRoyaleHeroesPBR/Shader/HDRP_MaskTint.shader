...
Shader "HDRP_MaskTint"
{
    ...
    void SurfaceDescription(
        ...
        inout SurfaceDescriptionOutputs OUT)
    {
        float4 baseColor = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, IN.uv0) * _BaseColor;
        OUT.BaseColor = baseColor.rgb;

        float3 emission = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, IN.uv0).rgb * _EmissionColor.rgb;
        OUT.EmissionColor = emission;
